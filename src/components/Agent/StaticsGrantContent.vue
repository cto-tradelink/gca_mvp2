<template>
    <div id="static_content">
        ...
        <div id="top_banner">
            <div class="on"> 
                <span class="title">진행중 사업(2)</span>
             
            </div>
            <div>
                <span class="title">총 등록사업(누적)(5)</span>
...
function make_pie_graph(target, arr,arr2){
        ...
        group.selectAll('path')
             ...
             for(var k=0; k < arr2.length;k++){
                 var $seg =  "<li><span style='background-color:"+colors(k)+"'></span><span>"+arr2[k]+"</span></li>"
        ...
        manager_sel:function(){
            ...
            for(var k =0; k < this.manager_list.length; k++){
                if( parseInt(this.manager_list[k].id) == parseInt($("#manager_sel").val()) ){
                    this.grant_list = this.manager_list[k].grant.slice()
                    console.log("find it!")
                    console.log(this.grant_list)
                }
            }}
}, 
    data:function(){
